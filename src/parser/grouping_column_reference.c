...
SqlStatement *grouping_column_reference(SqlStatement *derived_column_expression, SqlStatement *collate_clause) {
	...
	if (value_STATEMENT == derived_column_expression->type) {
		...
		invalid_syntax
		    = ((COLUMN_REFERENCE != value->type) && (TABLE_ASTERISK != value->type) && (INTEGER_LITERAL != value->type));
	} else {
		invalid_syntax = TRUE;
	}
	if (invalid_syntax)
	...
	cla->column_list = create_sql_column_list(derived_column_expression, NULL, &derived_column_expression->loc);
	return ret;
}
