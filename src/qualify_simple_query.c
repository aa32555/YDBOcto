 ...
 */
int qualify_simple_query(SqlStatement *simple_query_expr, SqlJoin *parent_join, SqlTableAlias *parent_table_alias,
			 QualifyStatementParms *ret) {

	int result = 0;

	if (set_operation_STATEMENT == simple_query_expr->type) {
		SqlSetOperation *set_opr;
		UNPACK_SQL_STATEMENT(set_opr, simple_query_expr, set_operation);
		result |= qualify_simple_query(set_opr->operand[0], parent_join, parent_table_alias, ret);
		result |= qualify_simple_query(set_opr->operand[1], parent_join, parent_table_alias, ret);
		return result;
	}
	...
	case table_value_STATEMENT:
		...
		SqlRowValue *start_row_value = row_value;
		do {
	 ...
	 */
	SqlJoin *prev_start = join;
	SqlJoin *prev_end = prev_start->prev;
	if (NULL != parent_join) {
	...
	/* Make sure to reset parent query only AFTER WHERE clause, etc. are processed.
