...
-- select samevalue(NOT 'test'=lastname) from names group by NOT 'test'=lastname having FALSE!=NOT 'test'=lastname; -- HAVING clause NOT expression not read well by parser OCTO
...
-- CREATE FUNCTION SAMEVALUE(INTEGER,FIRSTNAME) RETURNS VARCHAR AS $$samevalue^functions;

-- Postgres function
-- create function samevalue(integer,varchar) returns varchar
-- as 'select $1||$2;'
-- LANGUAGE SQL
-- IMMUTABLE;

-- select 1 from names n1 where exists(select samevalue(n1.id,n2.lastname) from names n2 group by samevalue(n1.id,n2.lastname));
-- SELECT ALL NOT (3 < customers.customer_id) FROM customers  WHERE EXISTS (SELECT MIN(ALL alias4.order_amount) as order_amount, alias4.order_id FROM orders alias4 WHERE ((alias4.order_date||'03/14/1760')<='07/21/179507/04/1776') GROUP BY alias4.order_id, samevalue(alias4.order_id, customers.first_name)) GROUP BY customers.customer_id;
