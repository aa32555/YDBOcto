...
select not firstname like 'Z%', count(firstname) from names group by not firstname like 'Z%';
select not firstname like 'Z%', count(firstname) from names group by firstname;
...
-- select firstname || 'x' as xyz, lastname || 'x' as lastname from names group by xyz, lastname || 'x'; -- Errors out --TODO: as GroupBy alias is not supported yet we need to check this usecase throughly to see how its error handling is done at present
...
-- misc
-- customers database
-- SELECT DISTINCT LEAST(alias1.customer_id,alias1.customer_id) FROM customers  LEFT JOIN orders AS alias1 ON (((customers.customer_id IS NOT NULL) OR (customers.address IS NULL))) WHERE (NOT NOT ((customers.city) IS NULL) AND ((('jadams@usa.gov') IS NULL))) ORDER BY LEAST(alias1.customer_id,alias1.customer_id);
