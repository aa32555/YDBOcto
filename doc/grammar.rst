...
The UNION operation consists of two or more queries joined together with the word UNION.  It combines the results of the individual queries into a single set of results.
...
The INTERSECT operation consists of two or more queries joined together with the word INTERSECT. It returns distinct results that are returned by both queries on either side of the operation.

The keyword ALL ensures that duplicate rows of results are not removed duing the INTERSECT.
...
The EXCEPT operation consists of two or more queries joined together with the word EXCEPT. It returns results from the query on the left side except those that are also part of the results from the query on the right side.

The keyword ALL ensures that duplicate rows of results are not removed during the EXCEPT operation.
